...
# =========================================================
# CMS A RESEARCH V1 — CORE vs ZONE
# IMPORTANT:
# 1) This is a research-only Streamlit app. It does NOT write A_Candidates,
#    A_AllScannedHistory, B_MasterList, or any production Sheet.
# 2) It NEVER calls Business Quant.
# 3) It NEVER calls Yahoo/yfinance for price history or sectors.
# 4) One click loads Supabase adjusted OHLCV once into st.session_state.
#    All comparisons in that session reuse the same locked snapshot.
# =========================================================

# Static sector mapping for the frozen 110-stock universe.
# This prevents the replay from calling Yahoo just to obtain sectors.
STATIC_SECTOR_MAP = {}

def _add_sector(names, sector):
    for _t in names:
        STATIC_SECTOR_MAP[_t] = sector

_add_sector([
    'AAPL','MSFT','NVDA','AVGO','AMD','ORCL','IBM','DELL','HPE','SMCI','CRM','ADBE','NOW','PLTR','PATH',
    'CRWD','PANW','FTNT','DDOG','NET','SNOW','MDB','ZS','OKTA','TEAM','QCOM','MU','INTC','ARM','MRVL','AMAT',
    'LRCX','KLAC','ON','MCHP','APP','IONQ','RGTI','SOUN'
], 'Technology')
_add_sector(['AMZN','TSLA','HD','UBER','ABNB','DASH','BKNG','MELI','RBLX','ROKU','DUOL','CAVA','CVNA'], 'Consumer Cyclical')
_add_sector(['META','GOOGL','NFLX','DIS','SPOT','RDDT'], 'Communication Services')
_add_sector(['JPM','BAC','WFC','GS','MS','V','MA','AXP','PYPL','COIN','HOOD','SOFI','XYZ','NU','IBKR','CRCL'], 'Financial Services')
_add_sector(['LLY','UNH','ABBV','MRK','AMGN','JNJ','PFE','GILD','ISRG','TMO','TEM','VEEV','REGN','VRTX','DXCM','HIMS'], 'Healthcare')
_add_sector(['XOM','CVX','COP','SLB'], 'Energy')
_add_sector(['CAT','GE','BA','RTX','LMT','ETN','VRT','RKLB','ASTS'], 'Industrials')
_add_sector(['PLUG','FCX'], 'Basic Materials')
_add_sector(['FSLR','CEG'], 'Utilities')
_add_sector(['WMT','COST'], 'Consumer Defensive')


def _score_structure_no_zone(df, price, atr14):
    """Zone-free replacement for Module 1, preserving the original 25-point weight.

    Removes every support/resistance/flip contribution. Keeps only:
      - prior 20-bar short-term breakout proximity (original max 5)
      - price compression/ATR compression (original max 8)
    The raw 13 points are rescaled to 25 so that downstream hard-filter/core
    thresholds remain comparable rather than being mechanically penalized.
    """
    high = pd.to_numeric(df['High'], errors='coerce')
    low = pd.to_numeric(df['Low'], errors='coerce')
    close = pd.to_numeric(df['Close'], errors='coerce')
    ...
    raw13 = short_score + compression_score
    score25 = int(round(raw13 / 13.0 * 25.0))
    score25 = max(0, min(25, score25))
    compression_ratio = r5 / r20 if r20 and not pd.isna(r20) else np.nan
...
def _hard_filter_with_structure(row, structure_score):
    """Same frozen hard filter, except Structure Score is supplied explicitly."""
    try:
        if row is None:
            return False
        if row['Price'] < 5:
            return False
        if row['Dollar Volume'] < 20_000_000:
            return False
        if row['Price'] < row['MA20'] * 0.99:
            return False
        if row['Price'] < row['MA50'] * 0.97:
            return False
        # MA200 intentionally NOT a hard veto.
        if pd.isna(row['MA20 Slope 5D']) or row['MA20 Slope 5D'] < 0.002:
            return False
        if pd.isna(structure_score) or structure_score < 8:
            return False
        return True
    except Exception:
        return False
...
def _base_resonance_buy(row):
    try:
        rn = safe_num(row.get('共振数'))
        return bool(
            rn >= 4
            and str(row.get('MACD共振')) == '是'
            and (str(row.get('量价共振')) == '是' or str(row.get('RS共振')) == '是')
        )
    except Exception:
        return False


def _analyze_one_date_research(ticker, hist, sector, benchmarks):
    """Run current A5.2R once, then derive two controlled counterfactuals."""
    cur = analyze_historical_a_core(ticker, hist, sector, benchmarks)
    if cur is None:
        return None

    # A. Current frozen A5.2R decision (includes current pressure-too-close veto).
    current_buy = bool(cur.get('Hard Filter') == '通过' and str(cur.get('A5决策')) == '买')

    # B. Same exact A5.2R core but remove only the final pressure/room veto.
    no_veto_buy = bool(cur.get('Hard Filter') == '通过' and _base_resonance_buy(cur))

    # C. Full No-Zone variant: support/resistance/flip removed from Module 1 AND no final zone veto.
    price = float(cur['Price'])
    atr14 = safe_num(cur.get('ATR14'))
    nz = _score_structure_no_zone(hist, price, atr14)
    nozone_hf = _hard_filter_with_structure(cur, nz['score'])
    nozone_buy = bool(nozone_hf and _base_resonance_buy(cur))

    out = dict(cur)
    out['Current A5.2R'] = current_buy
    out['No Space Veto'] = no_veto_buy
    out['No Zone Influence'] = nozone_buy
    out['NoZone Structure Score'] = nz['score']
    out['NoZone Structure Raw13'] = nz['raw13']
    out['NoZone Breakout'] = nz['Short-term Breakout']
    out['NoZone Compression Ratio'] = nz['Compression Ratio']
    return out


def _load_locked_snapshot():
    """Exactly one logical Supabase load for the complete 122-symbol snapshot."""
    tickers = get_universe()
    all_tickers = tuple(dict.fromkeys(tickers + BENCHMARK_TICKERS))
    data = supabase_batch_download(all_tickers)
    # Force independent in-memory copies so later research never refetches.
    locked = {t: _norm_daily_index(df.copy()) for t, df in data.items() if df is not None and not df.empty}
    return locked


def _future_labels_from_locked(full, asof, entry_price):
    if full is None or full.empty or entry_price <= 0:
        return None
    d = _norm_daily_index(full)
    future = d[d.index > pd.Timestamp(asof)].head(5)
    if len(future) < 5:
        return None
    highs = pd.to_numeric(future['High'], errors='coerce')
    if highs.isna().all():
        return None
    g1 = highs.iloc[:1].max() / entry_price - 1
    g3 = highs.iloc[:3].max() / entry_price - 1
    g5 = highs.iloc[:5].max() / entry_price - 1
    return {'1D Max Gain': g1, '3D Max Gain': g3, '5D Max Gain': g5}


def run_locked_research(snapshot, requested_days=30):
    tickers = get_universe()
    stock_data = {t: snapshot[t] for t in tickers if t in snapshot}
    bench_data = {t: snapshot[t] for t in BENCHMARK_TICKERS if t in snapshot}

    spy = bench_data.get('SPY')
    if spy is None or spy.empty:
        raise RuntimeError('Supabase快照没有SPY，无法回测。')

    spy_dates = list(_norm_daily_index(spy).index)
    # Need >=210 history on the as-of date and 5 future sessions for outcome labels.
    candidate_dates = spy_dates[209:-5] if len(spy_dates) >= 215 else []
    if not candidate_dates:
        raise RuntimeError('Supabase历史长度不足：至少需要215个交易日。')
    if requested_days == '全部可用':
        replay_dates = candidate_dates
    else:
        replay_dates = candidate_dates[-min(int(requested_days), len(candidate_dates)):]

    rows = []
    prog = st.progress(0)
    status = st.empty()
    total = max(1, len(replay_dates) * len(tickers))
    done = 0

    for di, asof in enumerate(replay_dates, start=1):
        status.write(f'回测 {pd.Timestamp(asof).date()} ({di}/{len(replay_dates)}) — 使用锁定快照，不再读取Supabase')
        benchmarks = _historical_benchmark_snapshot(bench_data, asof)
        for t in tickers:
            done += 1
            ...
            if len(hist) < 210:
                continue
            rec = _analyze_one_date_research(t, hist, STATIC_SECTOR_MAP.get(t, 'Unknown'), benchmarks)
            if rec is None:
                continue
            labels = _future_labels_from_locked(full, asof, float(rec['Price']))
            if labels is None:
                continue
            rec.update(labels)
            rec['Replay Date'] = pd.Timestamp(asof).strftime('%Y-%m-%d')
            rows.append(rec)
    prog.progress(100)
    status.empty()
...
def _variant_summary(df, col, label):
    x = df[df[col].fillna(False).astype(bool)].copy()
    g5 = pd.to_numeric(x.get('5D Max Gain'), errors='coerce').dropna()
    if len(g5) == 0:
        return {
            '版本': label, '候选数': 0, '交易日': 0, '平均每交易日候选': np.nan,
            '5D平均最大涨幅': np.nan, '5D中位数最大涨幅': np.nan,
            '≥3%': np.nan, '≥5%': np.nan, '≥8%': np.nan, '弱股<2%': np.nan,
        }
    return {
        '版本': label,
        '候选数': int(len(g5)),
        '交易日': int(x['Replay Date'].nunique()),
        '平均每交易日候选': len(g5) / max(1, x['Replay Date'].nunique()),
        '5D平均最大涨幅': float(g5.mean()),
        '5D中位数最大涨幅': float(g5.median()),
        '≥3%': float((g5 >= .03).mean()),
        '≥5%': float((g5 >= .05).mean()),
        '≥8%': float((g5 >= .08).mean()),
        '弱股<2%': float((g5 < .02).mean()),
    }


def _render_results(bt):
    if bt is None or bt.empty:
        st.warning('没有可用回测结果。')
        return

    st.header('结果：A-Core vs Zone')
    comp = pd.DataFrame([
        _variant_summary(bt, 'Current A5.2R', '① 当前 A5.2R（现有Zone/压力否决）'),
        _variant_summary(bt, 'No Space Veto', '② 只去掉“压力过近”最终否决'),
        _variant_summary(bt, 'No Zone Influence', '③ A-Core：Zone完全不参与选股'),
    ])
    st.dataframe(
        comp.style.format({
            '平均每交易日候选':'{:.2f}',
            '5D平均最大涨幅':'{:+.2%}','5D中位数最大涨幅':'{:+.2%}',
            '≥3%':'{:.1%}','≥5%':'{:.1%}','≥8%':'{:.1%}','弱股<2%':'{:.1%}'
        }, na_rep=''),
        hide_index=True, use_container_width=True
    )

    # False rejections caused specifically by the current final pressure veto.
    vetoed = bt[bt['No Space Veto'] & ~bt['Current A5.2R']].copy()
    vetoed['5D Max Gain'] = pd.to_numeric(vetoed['5D Max Gain'], errors='coerce')
    strong5 = vetoed[vetoed['5D Max Gain'] >= .05].copy()
    strong8 = vetoed[vetoed['5D Max Gain'] >= .08].copy()

    c1,c2,c3,c4 = st.columns(4)
    c1.metric('压力否决总数', len(vetoed))
    c2.metric('错杀 ≥5%', len(strong5))
    c3.metric('错杀 ≥8%', len(strong8))
    denom = max(1, len(vetoed))
    c4.metric('否决后却≥5%', f'{len(strong5)/denom:.1%}')

    st.subheader('被当前压力位挡掉、但后来实际上涨的股票')
    show_cols = [
        'Replay Date','Ticker','Price','A5.2R压力区','上方空间','压力测试次数_A52R',
        '5D Max Gain','共振数','MACD共振','量价共振','RS共振'
    ]
    show_cols = [c for c in show_cols if c in strong5.columns]
    if strong5.empty:
        st.success('这段样本中，没有发现被当前压力否决后仍达到 +5% 的股票。')
    else:
        st.dataframe(
            strong5.sort_values('5D Max Gain', ascending=False)[show_cols].style.format({
                'Price':'${:.2f}','上方空间':'{:+.2%}','5D Max Gain':'{:+.2%}'
            }, na_rep=''),
            hide_index=True, use_container_width=True
        )

    # Show what full no-zone changes versus current.
    added = bt[bt['No Zone Influence'] & ~bt['Current A5.2R']].copy()
    lost = bt[bt['Current A5.2R'] & ~bt['No Zone Influence']].copy()
    st.subheader('A-Core（完全去Zone）相对当前 A5.2R 的变化')
    d1,d2 = st.columns(2)
    with d1:
        st.metric('A-Core新增候选', len(added))
        if not added.empty:
            g = pd.to_numeric(added['5D Max Gain'], errors='coerce').dropna()
            st.caption(f"新增候选：≥5% {((g>=.05).mean() if len(g) else np.nan):.1%}；弱股<2% {((g<.02).mean() if len(g) else np.nan):.1%}")
    with d2:
        st.metric('A-Core丢失当前候选', len(lost))
        if not lost.empty:
            g = pd.to_numeric(lost['5D Max Gain'], errors='coerce').dropna()
            st.caption(f"丢失候选：≥5% {((g>=.05).mean() if len(g) else np.nan):.1%}；≥8% {((g>=.08).mean() if len(g) else np.nan):.1%}")

    st.subheader('结论提示')
    cur = comp.iloc[0]
    core = comp.iloc[2]
    if pd.notna(cur['5D平均最大涨幅']) and pd.notna(core['5D平均最大涨幅']):
        if (core['5D平均最大涨幅'] > cur['5D平均最大涨幅'] and core['≥5%'] >= cur['≥5%'] and core['弱股<2%'] <= cur['弱股<2%']):
            st.success('当前样本偏向 A-Core：去掉 Zone 后，收益/命中率没有恶化且弱股率不高。下一步应考虑把 Zone 从 A 选股中拆出，只服务 B 入场。')
        elif (cur['5D平均最大涨幅'] > core['5D平均最大涨幅'] and cur['≥5%'] >= core['≥5%'] and cur['弱股<2%'] <= core['弱股<2%']):
            st.info('当前样本偏向保留 Zone：现有 Zone 对 A 有净过滤价值。下一步再研究永久 Zone 是否能进一步改善。')
        else:
            st.warning('结果混合，没有明显赢家。先看“错杀≥5%/≥8%”明细，再决定是否扩大回测窗口。')


# =========================================================
# STREAMLIT UI
# =========================================================
st.set_page_config(page_title='CMS A Core vs Zone Research', page_icon='🧪', layout='wide')
st.title('🧪 CMS A — Core vs Zone 历史对照测试')
st.caption('研究专用：正式 A / B / C / Unified 全部不改。Business Quant = 0 次；Yahoo = 0 次。Supabase只在你点击“读取并锁定快照”时读取一次。')

st.info('比较三组：①当前A5.2R；②只取消“压力过近”最终否决；③A-Core，支撑/压力/翻转结构完全不参与Module 1和最终买入。其他MACD/KDJ/RSI/量价/RS规则保持不变。')

if 'research_snapshot' not in st.session_state:
    st.session_state['research_snapshot'] = None
if 'research_bt' not in st.session_state:
    st.session_state['research_bt'] = None

left,right = st.columns([1,2])
with left:
    if st.button('📥 读取并锁定 Supabase 快照（只做一次）', type='primary', use_container_width=True):
        try:
            with st.spinner('正在一次性读取122只股票/ETF的复权日K……'):
                snap = _load_locked_snapshot()
            st.session_state['research_snapshot'] = snap
            st.session_state['research_bt'] = None
            st.success(f'快照已锁定：{len(snap)}/122 个代码。后续回测不会再次读取 Supabase。')
        except Exception as e:
            st.error(f'读取Supabase失败：{e}')
with right:
    snap = st.session_state.get('research_snapshot')
    if snap:
        latest_dates = [df.index.max() for df in snap.values() if df is not None and not df.empty]
        earliest_dates = [df.index.min() for df in snap.values() if df is not None and not df.empty]
        st.success(f"当前内存快照：{len(snap)} 个代码；日期约 {min(earliest_dates).date()} → {max(latest_dates).date()}")
    else:
        st.warning('尚未读取快照。先点左边按钮。')

opts = [10,20,30,'全部可用']
replay_days = st.selectbox('回测最近多少个可验证交易日', opts, index=2)

if st.button('▶️ 运行 Core vs Zone 对照回测', use_container_width=True, disabled=not bool(st.session_state.get('research_snapshot'))):
    try:
        bt = run_locked_research(st.session_state['research_snapshot'], replay_days)
        st.session_state['research_bt'] = bt
        st.success(f'完成：{bt["Replay Date"].nunique() if not bt.empty else 0} 个交易日，{len(bt)} 个股票-日期样本。')
    except Exception as e:
        st.error(f'回测失败：{e}')

if isinstance(st.session_state.get('research_bt'), pd.DataFrame):
    _render_results(st.session_state['research_bt'])

st.caption('注意：A-Core 的 Structure 模块只保留“20日短线突破位置 + 压缩结构”，并把原13分同比例缩放回25分，以保持原Hard Filter/权重阈值可比较；这是研究设计，不会自动修改正式A。')
